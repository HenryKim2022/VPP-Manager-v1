                                ...
                                <tbody>
                                    @foreach ($loadDaftarLoginClientFromDB as $userLogin)
                                        <tr>
                                            ...
                                            {{-- <td>
                                                <div class="d-flex align-items-center justify-content-around">
                                                    <img src="{{ optional($userLogin->client)->foto_client ? asset('public/avatar/uploads/' . $userLogin->client->foto_client) : asset(env('APP_NOIMAGE')) }}"
                                                        alt="" style="height: 24px; width: 24px;" class="hover-qr-image">
                                                </div>
                                            </td> --}}
                                            <td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $(document).on('click', '.edit-record', function(event) {
                ...
                setTimeout(() => {
                    $.ajax({
                        url: '{{ route('m.user.client.getuser') }}',
                        method: 'POST',
                        headers: {
