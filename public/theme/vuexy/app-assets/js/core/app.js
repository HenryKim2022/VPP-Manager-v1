...
(function (window, document, $) {
    ...
    // \public\theme\vuexy\app-assets
    var assetPath = '../../../public/theme/vuexy/app-assets/';

    var $topnav = $('nav .header-navbar.navbar');
    var $sidenav = $('div .main-menu');
    ...
    $('.nav-link-style').on('click', function () {
        var currentLayout = getCurrentLayout(),
            switchToLayout = currentLayout === 'dark-layout' ? 'light-layout' : 'dark-layout';

        setLayout(switchToLayout);

        localStorage.setItem('layout', switchToLayout);
    });

    var currentLocalStorageLayout = localStorage.getItem('layout') || 'light-layout';           /// CHANGE THIS TO SET DEFAULT DARK/LIGHT MODE
    ...
    function setLayout(layout) {
        ...
        $html.removeClass('light-layout dark-layout bordered-layout semi-dark-layout');
        $html.addClass(layout);

        if (layout === 'dark-layout') {
            mainMenu.removeClass('menu-light').addClass('menu-dark');
            navbar.removeClass('navbar-light').addClass('navbar-dark');
            navLinkStyle.find('.ficon').replaceWith(feather.icons['sun'].toSvg({ class: 'ficon' }));
            $topnav.removeClass('navbar-light').addClass('navbar-dark');                        /// CHANGE THIS IF WANT USE DARK MODE AS DEFAULT (WONT)
            $sidenav.removeClass('menu-light').addClass('menu-dark');                           /// CHANGE THIS IF WANT USE DARK MODE AS DEFAULT (WONT BLINKING)

        } else {
            mainMenu.removeClass('menu-dark').addClass('menu-light');
            navbar.removeClass('navbar-dark').addClass('navbar-light');
            navLinkStyle.find('.ficon').replaceWith(feather.icons['moon'].toSvg({ class: 'ficon' }));
            $topnav.removeClass('navbar-dark').addClass('navbar-light');                        /// CHANGE THIS IF WANT USE DARK MODE AS DEFAULT (WONT BLINKING)
            $sidenav.removeClass('menu-dark').addClass('menu-light');                           /// CHANGE THIS IF WANT USE DARK MODE AS DEFAULT (WONT BLINKING)

        }

        if ($('input:radio[data-layout=' + layout + ']').length > 0) {
            setTimeout(function () {
                $('input:radio[data-layout=' + layout + ']').prop('checked', true);
            });
        }
    }
